...
class MisclassificationRateIV(Cost):
    def __init__(self, oos_thr=0., poos=0.23):
        self.oos_thr = oos_thr
        self.poos = poos
...
class OOSRateIV(Cost):
    @application(outputs=["oos_rate"])
    def apply(self, y, y_hat):
        # find the unlabeled samples: a combination of oos and labeled samples that were moved by +50
        unlabeled = (y >= y_hat.shape[1]).nonzero()
        y_hat = y_hat[unlabeled]
        oos = T.eq(y_hat.argmax(axis=1), 0)
        return T.mean(oos)

# Exactly like 160107-keras.ipynb
...
class CategoricalCrossEntropyIV(Cost):
    def __init__(self, Q, poos=0.23, alpha=0., beta=0.15, dbeta=0., gamma=0.01, gamma1=-1.):
        self.poos = poos
        self.alpha = alpha
        self.beta = beta
        self.dbeta = dbeta
        ...
        self.Q = Q
        P = (1.-poos)/Q*np.ones(Q+1)
        P[0] = poos
        P = P.reshape((1,-1))
        self.P = theano.shared(P.astype(theano.config.floatX), broadcastable=(True,False))

    @application(outputs=["cost"])
    def apply(self, y, y_hat):
        return T.sum(objective(y, y_hat, self.P, self.Q, self.alpha, self.beta, self.dbeta, self.gamma, self.gamma1))
